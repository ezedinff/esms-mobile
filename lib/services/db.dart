...
import './globals.dart';
import 'package:esms/services/database.dart';
...
class UserData<T> {
  ...
  Future<T> getDocument() async {
    ...
    if (user != null) {
      Document doc = Document<T>(path: '${user.uid}/$collection');
      return doc.getData();
    } else {
      return null;
    }
  ...
  Future<List<T>> getCollection() async {
    FirebaseUser user = await _auth.currentUser();
    if (user != null) {
      Collection col = Collection<T>(path: 'users/${user.uid}/$collection');
      return col.getData();
    } else {
      Collection col = Collection<T>(path: 'users/O94fsQuqaFVmHy6eU8RvzXJVCfx1/$collection');
      return col.getData();
    }
  ...
  Future<void> upsert(Map data) async {
    FirebaseUser user = await _auth.currentUser();
    Document<T> ref = Document(path:  '${user.uid}/$collection');
